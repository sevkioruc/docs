...
Permify unify your authorization data which behave as centralized data source used in access controls check 
...
Permify provides this YAML configuration file to point out where to store your authorization related data.
...
- **write:** Where your want to store relation tuples, audits , and decision logs.
...
## How to sync authorization data to WriteDB ?

After database configuration is done and Permify API is running as a REST API on your server. Relational tuples need to migrated or created in writedb. 

Additionally relational tuples can also be created, updated or deleted in runtime, since relations and authorization data's are live instances.

Permify provides a endpoint for creating relational tuples. Basically, you need to notify Permify API when user X is now owner of document Y by creating relational tuples with API request. 

## Creating Custom Relational Tuples

You can create custom relational tuples with using "/v1/relationships/write" endpoint.
